...
# Operate on our own copy, not the shared upstream contigs-db -- anvi-run-kegg-kofams
# mutates the db it's given in place (writes KOfam/KEGG_Module/KEGG_BRITE functions
# directly into it), and spa_coassembly_all/'s contigs-dbs are shared upstream source data,
# same "never mutate upstream" convention as everywhere else in this project.
BASE=/maps/projects/hansen_ol-AUDIT/scratch/NILU/metagenomes/assembly_annotation_wf/coassembly_production/anvio_kegg/contigs_dbs
